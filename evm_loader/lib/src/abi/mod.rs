...
async fn dispatch(method_str: &str, params_str: &str) -> Result<String, NeonError> {
    ...
    match method {
        ...
        LibMethods::Emulate => emulate::execute(&rpc, &config, params_str)
            .await
            .map(|v| serde_json::to_string(&v).unwrap()),
        LibMethods::GetNeonElf => get_neon_elf::execute(&rpc, &config, params_str)
            .await
            .map(|v| serde_json::to_string(&v).unwrap()),
        LibMethods::GetStorageAt => get_storage_at::execute(&rpc, &config, params_str)
            .await
            .map(|v| serde_json::to_string(&v).unwrap()),
        LibMethods::GetBalance => get_balance::execute(&rpc, &config, params_str)
            .await
            .map(|v| serde_json::to_string(&v).unwrap()),
        LibMethods::GetConfig => get_config::execute(&rpc, &config, params_str)
            .await
            .map(|v| serde_json::to_string(&v).unwrap()),
        LibMethods::GetContract => get_contract::execute(&rpc, &config, params_str)
            .await
            .map(|v| serde_json::to_string(&v).unwrap()),
        LibMethods::GetHolder => get_holder::execute(&rpc, &config, params_str)
            .await
            .map(|v| serde_json::to_string(&v).unwrap()),
